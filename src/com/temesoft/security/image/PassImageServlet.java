...
public class PassImageServlet extends HttpServlet {
	...
	private static final String SKEW_PROCESSOR_CLASS = Config
			.getProperty(Config.SKEW_PROCESSOR_CLASS);
		...
		try {
			...
			if (passedKeyStr == null) {
				ServletException se = new ServletException(
						"passedKeyStr is invalid");
				throw se;
			}
			if (passedKeyStr.length() < Config
			...
			ISkewImage skewImage = (ISkewImage) cl
					.loadClass(
							sprocessorClasses[(int) (Math.random() * sprocessorClasses.length)])
					.newInstance();
			BufferedImage bufferedImage = skewImage.skewImage(passedKeyStr
					.substring(0, passedKeyStr.indexOf(".")));
