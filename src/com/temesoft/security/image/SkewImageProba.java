 ...
 * /</a> <br>
 * <b>This is a sample implementation of the ISkewImage class in order to skew
 * the secured chars passed encoded as a parameter</b>
 */
public class SkewImageProba implements ISkewImage {
	...
	// Color.RED, Color.CYAN, Color.GREEN, Color.MAGENTA, Color.ORANGE,
	// Color.PINK,
	// Color.YELLOW};

	private static final Color[] RANDOM_BG_COLORS = { Color.WHITE, Color.WHITE,
			Color.WHITE, Color.WHITE, Color.WHITE, Color.WHITE, Color.WHITE };

	private static final Color[] RANDOM_FG_COLORS = { Color.BLACK, Color.BLUE,
			Color.DARK_GRAY };
	...
	public BufferedImage skewImage(String securityChars) {
		...
		for (int i = 0; i < MAX_LETTER_COUNT; i++) {
			...
			affineTransform.rotate(angle, (LETTER_WIDTH * i)
					+ (LETTER_WIDTH / 2), MAX_Y / 2);
		...
		g2d
				.setXORMode(RANDOM_BG_COLORS[(int) (Math.random() * RANDOM_BG_COLORS.length)]);
	...
	/*
	 * private void setRandomFont(Graphics2D g2d) { Font font = new
	 * Font("dialog", 1, 33); g2d.setFont(font); }
	 * 
	 * private void setRandomFGColor(Graphics2D g2d) { int colorId =
	 * (int)(Math.random() RANDOM_FG_COLORS.length);
	 * g2d.setColor(RANDOM_FG_COLORS[colorId]); }
	 * 
	 * private void setRandomBGColor(Graphics2D g2d) { int colorId =
	 * (int)(Math.random() RANDOM_BG_COLORS.length);
	 * g2d.setColor(RANDOM_BG_COLORS[colorId]); }
	 */
}
