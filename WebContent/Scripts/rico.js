...
var Rico = {
  ...
  init : function() {
    ...
    for (var i=0; i<elements.length; i++) {
      ...
      if (filename == 'rico.js') {
        var isRailsPath = (path.indexOf("/Scripts") > 0)
        if (isRailsPath){
          this.jsDir = "../Scripts/";
          this.cssDir = "../css/";
          this.imgDir = "../images/";
    ...
    if (typeof Ajax=='undefined') this.include('ajax.js');
    if (typeof Prototype=='undefined') this.include('prototype.js');
    this.includeLoaded('ricoCommon.js');
    this.includeLoaded('ricoStyles.js');
    //this.includeLoaded('ajax.js');
  ...
  include : function(filename) {
    ...
    switch (ext.toLowerCase()) {
      case 'js':
        ...
        //alert("<script type='text/javascript' src='"+this.jsDir+filename+"'><\/script>");
  ...
  //alert("called after a script file has finished loading");
    this.loadedFiles[filename]=this.loadComplete;
